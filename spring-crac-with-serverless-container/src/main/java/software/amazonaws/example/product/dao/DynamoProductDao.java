...
public class DynamoProductDao implements ProductDao {
	...
	private static final SdkHttpClient httpClientForPriming = ApacheHttpClient.builder()
			.connectionTimeToLive(Duration.ofSeconds(1)).socketTimeout(Duration.ofSeconds(1))
			.connectionTimeout(Duration.ofSeconds(1)).build();
	...
	private static DynamoDbClient getDynamoDbClient(SdkHttpClient httpClient, boolean setEndPoint) {
		if (ENDPOINT != null && setEndPoint) {
			return DynamoDbClient.builder().endpointOverride(URI.create(ENDPOINT))
					...
					.httpClient(httpClient).overrideConfiguration(ClientOverrideConfiguration.builder().build())
			...
			return DynamoDbClient.builder().credentialsProvider(DefaultCredentialsProvider.create())
					...
					.overrideConfiguration(ClientOverrideConfiguration.builder().build()).httpClient(httpClient)
					.build();
		}
	}
	...
	public Products getAllProduct() {
		ScanResponse scanResponse = dynamoDbClient
				.scan(ScanRequest.builder().tableName(PRODUCT_TABLE_NAME).limit(20).build());
